...
static int interaction(all_t *all, mob_t *john, mission_enum_t mission, int i)
{
    if (KEY_RELEASE && KEY(sfKeyE) && MISSION1 == mission &&
    dist_btw_sprite(PLAYER_S, john->pnj->sprite) <= DIST_INTERACTION &&
    john->i == i) {
        return 1;
    }
    return 0;
...
void end_mission1(all_t *all, mob_t *john)
{
...
void in_parc(all_t *all, mob_t *john)
{
    ...
    if (MISSION1 == start) {
        if (FIGHT->fight_result.player_win == true) {
            MISSION1 = end_figth;
            sfSprite_setPosition(john->pnj->sprite, v2f(2900, 100));
...
static void my_mission_animation(all_t *all, mob_t *john)
{
    sfVector2f dest = {0, 0};
...
void mission_1(all_t *all, mob_t *john)
{
    if (!john || !john->pnj || !john->pnj->sprite || MISSION2 != m_none)
        return;
